defmodule PearlWeb.Backoffice.EventLive.FaqLive.FormComponent do
  ...
  defp save_faq(socket, :faqs_edit, faq_params) do
    case Event.update_faq(socket.assigns.faq, faq_params) do
      {:ok, _faq} ->
        {:noreply,
         socket
         |> put_flash(:info, gettext("FAQ atualizada com sucesso."))
  ...
  defp save_faq(socket, :faqs_new, faq_params) do
    case Event.create_faq(faq_params) do
      {:ok, _faq} ->
        {:noreply,
         socket
         |> put_flash(:info, gettext("FAQ criada com sucesso."))
         |> push_patch(to: socket.assigns.patch)}
